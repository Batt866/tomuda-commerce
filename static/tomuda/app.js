...
.receipt-grid__a { width: 5.2%; } .receipt-grid__b { width: 16.8%; } .receipt-grid__c { width: 9.2%; } .receipt-grid__d { width: 3.4%; } .receipt-grid__e { width: 13.3%; }
...
.receipt-grid__summary-label--grand { font-weight: 700; color: ${RECEIPT_TEXT} !important; font-size: 11px !important; letter-spacing: 0; white-space: nowrap; }
...
// ҮНДСЭН A–K: A logo бага зэрэг өргөн; B:C bank labels (incl. Дансны дугаар)
const RECEIPT_XLSX_COL_WIDTHS = [
  4.5, 15.0, 9.0, 2.875, 12.0, 9.25, 5.5, 4.875, 3.75, 9.25, 9.5,
];
...
function receiptDrawingXml() {
  ...
  // ~5.5mm into B (A≈8mm + 5.5mm ≈ 13.5mm logo)
  const intoB = Math.round((5.5 / 25.4) * 914400);
...
function appendReceiptSheetRows(o, ctx, rows, merges, startRow = 1) {
  ...
  merges.push(
    ...
    `B${bankR3}:C${bankR3}`,
    `D${bankR3}:E${bankR3}`,
    // Same as other bank labels: B:C = «Дансны дугаар:»; D:E = «IBAN:» + short number
    `B${bankR4}:C${bankR4}`,
    `D${bankR4}:E${bankR4}`,
    `D${bankR5}:E${bankR5}`,
  );
