...
const RECEIPT_BORDER = "#666666";
const RECEIPT_HEADER_BG = "#F3F3F3";
const RECEIPT_GRAND_BG = "#C5CAD0";
...
/** CustomerInformation — PDF column proportions, full visible addresses. */
/** CustomerInformation — 11-col grid matching ҮНДСЭН sample. */
...
function CustomerInformation(o) {
  return receiptInfoRows(o);
}
...
function receiptPaymentChecksText(paid, bank, o) {
  return o ? receiptPaymentTermReceiptText(o) : bank ? "Зээлээр" : "Шууд төлөх";
}
...
/** ProductTable — item rows on the same 11-col sheet (no nested table). */
function receiptItemsColgroupHtml() {
  return "";
}
...
function ProductTable(o, items, startIndex = 0, withHead = true) {
  return receiptItemsBlockHtml(o, items, startIndex, withHead);
}
...
function receiptPromoBarcode(item) {
  const p = productForReceiptLine(item);
  const code = String(p.barcode || item.barcode || "").trim();
  return code || "-";
}
/** RewardTable — ҮНДСЭН: Урамшуулал in C:D, name E:G, qty H:I, price J, total K (no settle above). */
  ...
  return (
    promoItems
      .map((i, idx) => {
        ...
        return `<tr class="receipt-items__promo"><td></td><td></td><td colspan="2"></td><td colspan="3" class="receipt-items__promo-name">${name}</td><td colspan="2" class="receipt-items__qty">${qty}</td><td class="receipt-items__price">${price}</td><td class="receipt-items__total">${total}</td></tr>`;
      })
      .join("") +
    `<tr class="receipt-grid__spacer receipt-grid__spacer--promo-gap"><td colspan="11"></td></tr>`
  );
}
function RewardTable(o) {
  return receiptPromoRowsHtml(o);
}
function receiptGrossRowHtml(o) {
  const gross = orderGrossTotal(o);
  // ҮНДСЭН: label B:D, underline E:K (amount in K) — line must reach past Тоо/ш.
  return `<tr class="receipt-grid__gross"><td></td><td colspan="4" class="receipt-grid__gross-label">Хувь хасагдаагүй нийт үнийн дүн</td><td colspan="5" class="receipt-grid__gross-rule"></td><td class="receipt-grid__money receipt-grid__money--strong">${receiptMoney(gross)}</td></tr>`;
}
/** SummarySection — grand → cash settle note → pay term (cash note above term). */
function receiptSummaryRowsHtml(sub, vat, payable, payTerm, o) {
  ...
  const settleRow = receiptShouldShowCashSettleNote(o)
    ? `<tr class="receipt-grid__settle-note"><td></td><td colspan="10" class="receipt-grid__settle-note-text">${esc(receiptCashSettleNoteText())}</td></tr>`
    : "";
  return `${subRow}${vatRow}${grandRow}${settleRow}${payRow}`;
}
...
function receiptGrandNote(o) {
  ...
  const pct =
    o.applyPercentDiscount && receiptIsDirectPay(o)
...
/** Settle warning — cash / Шууд төлөх only (same rule as gross row). */
function receiptShouldShowCashSettleNote(o) {
  return receiptIsDirectPay(o);
}
function receiptCashSettleNoteText() {
  return "Барааг хүлээн авсан өдөртөө төлбөрөө барагдуулаагүй тохиолдолд хувь хасагдаагүй дүнгээр тооцохыг анхаарна уу!";
}
/** True when the receipt prints «Шууд төлөх» (not «Зээлээр»). */
function receiptIsDirectPay(o) {
  if (!o) return false;
  if (o.paymentTerm === "credit") return false;
  if (o.paymentTerm === "cash") return true;
  // Missing/legacy terms: follow the same label used on the receipt.
  return receiptPaymentTermDisplay(o) === "Шууд төлөх";
}
...
function PaymentNotice(o) {
  return `<tr class="receipt-grid__settle"><td></td><td colspan="10" class="receipt-grid__settle-text">${esc(receiptSettleNoteText(o))}</td></tr>`;
}
...
function receiptNoteText(o) {
  return String(o?.settlementText || "").trim();
}
function receiptUpperFooterRows(o) {
  // Зөвхөн захиалгын тэмдэглэлийн текст; «Тэмдэглэл бичих хэсэг:» шошгогүй.
...
function receiptLowerFooterRows(o, opts = {}) {
  ...
  const grossHtml = receiptShouldShowGross(o) ? receiptGrossRowHtml(o) : "";
...
function orderMatchesWarehouseDate(o, day = state.filters.warehouseDate) {
  ...
  // Агуулах / Баримтууд жагсаалт = авсан өдөр. Хүргэлтийн өдрөөр бүү шүү.
...
function customerHistoryDateFieldHtml(label, value, onChange, ariaLabel) {
  ...
  const display = iso ? dte(iso) : "Сонгох";
  return `<label class="customer-history__date"><span class="customer-history__date-label">${label}</span><span class="customer-history__date-shell"><span class="customer-history__date-value${iso ? "" : " is-empty"}">${esc(display)}</span><input type="date" class="customer-history__date-native" value="${esc(iso)}" onchange="${onChange}" aria-label="${esc(ariaLabel)}"></span></label>`;
}
...
function importUploadFilename(file, kind) {
  ...
  const base =
    raw.replace(/\.[^.]+$/, "") ||
    (kind === "customers" ? "Харилцагч" : "Бараа");
  return `${base}.xlsx`;
}
...
function importTemplateFilename(kind) {
  return kind === "customers" ? "Харилцагч-формат.xlsx" : "Бараа-формат.xlsx";
}
...
function sidebarNavForRole(role) {
  ...
  // Агуулах = бараа бэлдэх (warehouse). Нярав = үлдэгдэл / орлого / зарлага.
...
function releaseWarehouseDatePicker() {
...
/** How many жижиг хайрцаг fit in one том хайрцаг. */
function productLargeBoxCount(p) {
  const n = Number(p?.largeBoxQuantity);
  return Number.isFinite(n) && n > 1 ? Math.floor(n) : 0;
}
/** Pieces in one том хайрцаг (= том × жижиг × ширхэг). */
...
function warehousePrepareLargeSize(item, product) {
  ...
  const largeCount =
    Number(item?.largeBoxQuantity) > 1
      ? Math.floor(Number(item.largeBoxQuantity))
      : productLargeBoxCount(product);
  if (packSize > 1 && largeCount > 1) return packSize * largeCount;
...
function syncPickerQtySheetUi(id) {
  ...
  if (workerSum && p) {
    const parts = pickerQtyToParts(q, p);
    workerSum.innerHTML = `${prepareQtyColsHtml(parts, { product: p })}<span class="worker-pack-edit__pcs">· ${q} ш</span>`;
  }
...
function workerPackQtyEditHtml(p, q) {
  ...
  return `<div class="worker-pack-edit" data-worker-pack-edit="${esc(p.id)}">${rows.join("")}<div class="worker-pack-edit__sum">${partsLabel}<span class="worker-pack-edit__pcs">· ${q} ш</span></div><button type="button" class="worker-row-done-link" onclick="finishWorkerOrderEdit()">Болсон</button></div>`;
}
function workerQtyStepperHtml(p, q) {
  const idAttr = esc(p.id);
  const maxOk = maxWorkerPaidQty(p.id);
  const minQty = state.editingOrderId ? 0 : 1;
  const decDisabled = q <= minQty;
  const incDisabled = q >= maxOk;
  return `<div class="qty-stepper worker-order-qty-stepper" data-qty-min="${minQty}"><button type="button" class="qty-stepper__btn qty-stepper__btn--dec" data-qty-action="dec" data-product-id="${idAttr}" ${decDisabled ? "disabled" : ""} aria-label="Багасгах">−</button><input data-product-id="${idAttr}" oninput="qtyDraft(this)" onblur="qtyCommit(this)" value="${q}" type="tel" inputmode="numeric" pattern="[0-9]*" autocomplete="off" class="app-input qty-stepper__input" aria-label="Тоо ширхэг"><button type="button" class="qty-stepper__btn qty-stepper__btn--inc" data-qty-action="inc" data-product-id="${idAttr}" ${incDisabled ? "disabled" : ""} aria-label="Нэмэх">+</button></div>`;
}
...
function workerOrderQtyHtml(p, q) {
  ...
  const qtyHtml =
    productPackSize(p) || productLargeBoxPieceCount(p)
      ? prepareQtyColsHtml(
          {
            largePacks: parts.largePacks,
            packs: parts.packs,
      ...
      : `<span class="worker-row-qty__n">${q} ${esc(p.unit || "ш")}</span>`;
  return `<div class="worker-row-qty-wrap"><button type="button" class="worker-row-qty" onclick="setWorkerOrderActive('${id}')">${qtyHtml}</button>${removeBtn}</div>`;
}
...
function go(view, opts = {}) {
  ...
  if (changed && view === "warehouse") {
    ...
    // Агуулах opens бараа бэлдэх. Орлого/Зарлага live under Нярав.
  ...
  if (changed && view === "inventory") {
    if (!opts.keepInventoryTab) {
      state.filters.inventory = "stock";
    } else if (
      state.filters.inventory !== "in" &&
      state.filters.inventory !== "out" &&
      state.filters.inventory !== "stock"
    ) {
      state.filters.inventory = "stock";
    }
...
.receipt-grid--sheet tr.receipt-grid__items-wrap > td {
...
.receipt-grid--sheet tr.receipt-items__row > td.receipt-items__qty { text-align: center; }
.receipt-grid--sheet tr.receipt-items__row > td.receipt-items__qty { font-weight: 700; }
.receipt-grid--sheet tr.receipt-items__row > td.receipt-items__barcode {
  text-align: center;
  font-size: 9px;
  word-break: break-all;
}
.receipt-grid--sheet tr.receipt-items__row > td.receipt-items__price,
...
.receipt-grid--sheet tr.receipt-grid__info-wrap > td.receipt-grid__info-cell { padding: 1px 0 !important; }
.receipt-info {
  width: 100%;
  box-sizing: border-box;
...
.receipt-info__party { width: 100%; margin: 0 0 4px; }
.receipt-info__pair {
  display: grid;
...
.receipt-info__row td { padding-top: 0; padding-bottom: 2px; vertical-align: top; line-height: 1.25; }
...
.receipt-items--promo .receipt-items__promo-settle-row td {
  border: none !important;
  border-bottom: 0.4pt solid #777 !important;
...
.receipt-items--promo .receipt-items__promo-barcode {
...
.receipt-grid__header--title td { padding-top: 2px !important; padding-bottom: 2px !important; }
...
.receipt-grid__label { color: ${RECEIPT_TEXT}; white-space: nowrap; font-weight: 400; font-size: 9px; }
...
.receipt-grid__spacer--note td { height: 14px; padding: 0; }
...
.receipt-grid__gross .receipt-grid__money {
  font-size: 11px !important;
  font-weight: 700 !important;
  text-align: right;
}
...
.receipt-grid__summary-label--grand { font-weight: 700; color: ${RECEIPT_TEXT} !important; font-size: 9px !important; letter-spacing: 0; white-space: nowrap; text-align: right !important; }
...
.receipt-grid__warn-line { margin: 0 0 1px; font-size: 8px; line-height: 1.2; color: ${RECEIPT_TEXT}; font-weight: 400; }
...
function exportOrderReceiptsExcelCsv(orders) {
  ...
  excel(`Захиалгын-баримт-${stamp}.xlsx`, sheetRows);
}
const RECEIPT_XLSX_LAST_COL = "K";
/** Canonical ҮНДСЭН layout (BIFF). Runtime Excel mirrors this sample. */
const RECEIPT_XLSX_SOURCE_TEMPLATE =
  "/static/tomuda/templates/zarlaga-receipt-undsen.xls";
...
function receiptXlsxLabelUnits(text) {
  ...
  for (const ch of String(text ?? "")) {
    ...
    else if (/[\u0400-\u04FF]/.test(ch)) units += 1.12;
    else units += 0.95;
  }
  return units;
}
...
function receiptXlsxWrappedRowHeight(
  ...
  // Numbers/Excel fit more Arial-9 glyphs than raw customWidth units.
  // Keep mid-length names (…/Гүзээлзгэнэ/) on one line — too-low limit made empty 2-line rows.
  const charsPerLine = Math.max(
    10,
    Math.round((Number(colWidth) || 18) * 1.35),
  );
...
function filterXlsxCellsOutsideMerges(cells, mergeRefs) {
  // Keep merge-covered cells (C/D inside B:D, G inside F:G, I inside H:I, …).
  // Excel/Numbers often draw top/bottom borders only as wide as the *written*
  // cells — dropping G/I made barcode lines stop before Тоо/ш, and summary
  // underlines stop mid-row. Styles on covered cells match the ҮНДСЭН sample.
  void mergeRefs;
  const seen = new Set();
  const out = [];
...
function appendReceiptSheetRows(o, ctx, rows, merges, startRow = 1) {
  ...
  const bankR1 = rowNum;
  const bankR2 = bankR1 + 1;
  const bankR3 = bankR1 + 2;
  const bankR4 = bankR1 + 3;
  const bankR5 = bankR1 + 4;
  merges.push(
    `B${bankR1}:C${bankR1}`,
    `D${bankR1}:E${bankR1}`,
    `F${bankR1}:H${bankR1}`,
  ...
  const perBankH = Math.max(14.25, Math.ceil(addrH / 4));
  pushRow(perBankH, [
  ...
  const nameColWidth =
    ...
    RECEIPT_XLSX_COL_WIDTHS[2] +
    RECEIPT_XLSX_COL_WIDTHS[3];
  const barcodeColWidth =
    RECEIPT_XLSX_COL_WIDTHS[5] + RECEIPT_XLSX_COL_WIDTHS[6];
  ...
  // Тэмдэглэл — зөвхөн бичсэн текст (шошгогүй).
  ...
  // Gap before totals — keep normal height; empty rows can collapse in Excel/Numbers.
  {
    const spacerR = rowNum;
    pushRow(18, [
      xlsxCellXml(`A${spacerR}`, 1, si("\u00A0"), "s"),
      ...emptyCells(spacerR, "B", RECEIPT_XLSX_LAST_COL, 1),
    ]);
  }
  ...
  if (receiptShouldShowGross(o)) {
    ...
    pushRow(14.25, [
      ...
      xlsxCellXml(`F${grossRow}`, 32, null, "empty"),
      xlsxCellXml(`G${grossRow}`, 32, null, "empty"),
      xlsxCellXml(`H${grossRow}`, 32, null, "empty"),
      xlsxCellXml(`I${grossRow}`, 32, null, "empty"),
      xlsxCellXml(`J${grossRow}`, 32, null, "empty"),
      xlsxCellXml(`K${grossRow}`, 32, Number(gross) || 0, "n"),
      ...emptyCells(grossRow, "C", "E", 19),
      ...emptyCells(grossRow, "A", "A", 1),
    ]);
  }
  ...
  promoLines.forEach((item, idx) => {
    ...
    pushItemTableRow(promoH, [
      ...
      xlsxCellXml(`K${r}`, moneyStyle, Number(lineTotal) || 0, "n"),
      ...emptyCells(r, "D", "D", labelStyle),
      ...emptyCells(r, "F", "G", bodyStyle),
      ...emptyCells(r, "I", "I", qtyStyle),
    ]);
  });

  // Extra gap after promo so summary doesn't sit on the promo border
...
function productDetailHtml(p, id) {
  ...
  if (largeCount && packSize) {
    rows.push(
      productDetailRow(
        "Том хайрцаг",
        `${largeCount} жижиг/том (= ${largePieces} ш)`,
      ),
    );
  }
  if (packSize) {
  ...
  return `<div class="space-y-4">${pageHead("Нярав")}${
    tabs.length > 1
      ? `<div class="${tabClass}">${tabs
          ...
          .join("")}</div>`
      : ""
  }${inventoryRegisterBody(tab)}</div>`;
}
function setWarehouseTab(tab) {
  // Агуулах is бараа бэлдэх only; орлого/зарлага open under Нярав.
  if (tab === "in" || tab === "out") {
    setInventoryTab(tab);
    return;
  }
...
function setInventoryTab(tab) {
  ...
  if (state.currentView !== "inventory") {
    go("inventory", { keepInventoryTab: true });
    return;
  }
  render();
  scrollAppMainToTop();
...
function stockInCostPriceWarn(el, salesPrice) {
  ...
  const warn = form?.querySelector("[data-stock-in-cost-warn]");
  if (warn) {
...
function stockInTakeIcon(kind) {
  const common =
    'class="stock-in-take__svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"';
  if (kind === "box") {
    return `<svg ${common}><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><path d="M3.3 7 12 12l8.7-5"/><path d="M12 22V12"/></svg>`;
  }
  if (kind === "piece") {
    return `<svg ${common}><rect x="4" y="6" width="16" height="12" rx="1.5"/><path d="M4 10h16"/></svg>`;
  }
  if (kind === "sum") {
    return `<svg ${common}><path d="M6 6h12M6 12h12M6 18h12"/><path d="M9 9v0M9 15v0"/></svg>`;
  }
  if (kind === "tag") {
    return `<svg ${common}><path d="M20.6 13.4 12.4 21.6a2 2 0 0 1-2.8 0L2.4 14.4a2 2 0 0 1 0-2.8L10.6 3.4A2 2 0 0 1 12 3h6a2 2 0 0 1 2 2v6a2 2 0 0 1-.4 1.4z"/><circle cx="16" cy="8" r="1.2" fill="currentColor" stroke="none"/></svg>`;
  }
  if (kind === "calc") {
    return `<svg ${common}><rect x="4" y="3" width="16" height="18" rx="2"/><path d="M8 7h8M8 12h2M12 12h2M16 12h0M8 16h2M12 16h2M16 16h0"/></svg>`;
  }
  if (kind === "save") {
    return `<svg ${common}><path d="M12 3v12"/><path d="m7 10 5 5 5-5"/><path d="M5 19h14"/></svg>`;
  }
  return "";
}
function stockInTakeRowHtml({
  icon,
  label,
  name,
  value = "",
...
function stockInQtyFieldsInput(_el) {
  ...
  const costTyped = String(costRaw || "").trim() !== "";
  const priorCost = Math.max(
    0,
    Math.floor(Number(root.dataset.priorCost || 0) || 0),
  );
  ...
  const unitCost = costTyped
    ...
    : priorCost;
  const totalCost = total > 0 && unitCost > 0 ? total * unitCost : 0;
  if (costInput) {
    costInput.classList.toggle(
      "stock-in-take__input--soft",
      !costTyped && priorCost > 0,
    );
  }
  if (largePreview) {
    if (lp > 0 && largeCount) {
...
function stockTakeEntryFormHtml(p, d = {}, mode = "in") {
  ...
  const noteVal = d.note != null && d.note !== "" ? String(d.note) : "";
  ...
  const effectiveUnitCost = costVal
    ...
    : priorCost;
  const initialCost =
    !isOut && initialTotal > 0 && effectiveUnitCost > 0
      ? initialTotal * effectiveUnitCost
      : 0;
  const packRules = [
    smallSize
      ? `<p class="stock-in-take__rule">1 жижиг хайрцаг = <b>${smallSize}</b> ширхэг</p>`
      : "",
    largeCount && smallSize
      ? `<p class="stock-in-take__rule">1 том хайрцаг = <b>${largeCount}</b> жижиг хайрцаг (<b>${largePieces}</b> ширхэг)</p>`
      : "",
    isOut
      ? `<p class="stock-in-take__rule">Үлдэгдэл: <b>${Number(p.stock) || 0}</b> ${esc(p.unit || "ш")} · оруулсан тоо хасагдана</p>`
      : !smallSize && !largePieces
        ? `<p class="stock-in-take__rule">Үлдэгдэл: <b>${p.stock ?? 0}</b> ${esc(p.unit || "ш")}</p>`
        : "",
  ]
    .filter(Boolean)
    .join("");
  ...
  if (largePieces) {
    qtyRows.push(
      stockInTakeRowHtml({
        icon: stockInTakeIcon("box"),
        label: "Том хайрцагны тоо",
        name: "largePacks",
        value: largeVal,
  ...
  if (!isOut) {
    qtyRows.push(
      stockInTakeRowHtml({
        ...
        label: "Нэгжийн өртөг үнэ",
        name: "costPrice",
        value: costVal,
        suffix: "₮",
        soft: !costVal && priorCost > 0,
        placeholder: priorCost ? String(priorCost) : "0",
        oninput: `stockInCostPriceWarn(this, ${salesPrice})`,
        ariaLabel: "Нэгжийн өртөг үнэ",
      }),
    );
  }
...
function stockInEntryModal(id) {
  ...
  box(
    "Орлого авах",
    stockTakeEntryFormHtml(p, d, "in"),
    "max-w-md stock-in-take-modal",
  );
  requestAnimationFrame(() => {
    const form = modal.querySelector("form.stock-in-take");
    const large = form?.querySelector('input[name="largePacks"]');
...
function stockInReceiptTitle(receipt) {
  const no = receipt?.receiptNumber;
  return no ? `Орлогын баримт №${no}` : "Орлогын баримт";
}
...
function applyStockInEntryModal(e, id) {
  ...
  if (costRaw !== "" && stockInCostExceedsSalesPrice(costPrice, p)) {
...
function stockReceiptPackHtml(line) {
  const p =
    state.products.find((x) => String(x.id) === String(line?.productId)) ||
    null;
  const largeCount =
    Number(line?.largeBoxQuantity) > 1
      ? Math.floor(Number(line.largeBoxQuantity))
  ...
  const bits = [];
  if (excl.largePacks) {
    bits.push(`<span class="stock-in-pack-chip">${excl.largePacks} том</span>`);
  }
  if (excl.packs) {
    bits.push(`<span class="stock-in-pack-chip">${excl.packs} жижиг</span>`);
  }
  if (!bits.length) {
    return `<span class="stock-in-pack-chip stock-in-pack-chip--empty">—</span>`;
  }
  return `<span class="stock-in-pack">${bits.join("")}</span>`;
}
function stockInReceiptRow(line) {
  ...
  const costCell = showCost
    ? `<span class="stock-in-receipt-line__price"><span class="stock-in-receipt-line__price-label">Өртөг</span><span class="stock-in-receipt-line__price-value">${fmt(costPrice)}</span></span>`
    : "";
  return `<article class="stock-in-receipt-line"><div class="stock-in-receipt-line__main"><p class="stock-in-receipt-line__name">${esc(line.productName)}</p>${noteHtml}<div class="stock-in-receipt-line__meta"><span class="stock-in-receipt-line__barcode">${esc(line.barcode || "—")}</span>${stockReceiptPackHtml(line)}<span class="stock-in-receipt-line__qty">${line.quantity} ш</span></div></div><div class="stock-in-receipt-line__prices">${costCell}<span class="stock-in-receipt-line__price"><span class="stock-in-receipt-line__price-label">Үнэ</span><span class="stock-in-receipt-line__price-value">${fmt(salesPrice)}</span></span><span class="stock-in-receipt-line__price stock-in-receipt-line__price--total"><span class="stock-in-receipt-line__price-label">Нийт</span><span class="stock-in-receipt-line__price-value">${fmt(totalPrice)}</span></span></div></article>`;
}
...
function stockInCategoryTotalRow(name, amount) {
  return `<div class="stock-in-receipt-subtotal"><span class="stock-in-receipt-subtotal__label">${esc(name)} нийт</span><span class="stock-in-receipt-subtotal__value">${fmt(amount)}</span></div>`;
}
function stockInReceiptPanel(receipt) {
  ...
  const rows = groups
    .map((item) => {
      if (item.type === "cat") {
        return `<div class="stock-in-receipt-cat">${esc(item.name)}</div>`;
      }
  ...
  return `<section class="stock-in-receipt-panel"><header class="stock-in-receipt-panel__head"><div><p class="stock-in-receipt-panel__title">${esc(stockInReceiptTitle(receipt))}</p><p class="stock-in-receipt-panel__sub">Ажилтан: ${esc(receipt.employeeName)} · ${receipt.lines.length} бараа</p></div></header><div class="stock-in-receipt-list">${rows}<div class="stock-in-receipt-total"><span class="stock-in-receipt-total__label">Нийт дүн</span><span class="stock-in-receipt-total__value">${fmt(receipt.totalAmount)}</span></div></div><footer class="stock-in-receipt-panel__signatures"><div class="stock-in-sign"><span>Хүлээлгэн өгсөн</span><span class="stock-in-sign__line"></span></div><div class="stock-in-sign"><span>Хүлээн авсан</span><span class="stock-in-sign__line"></span></div><div class="stock-in-sign stock-in-sign--date"><span>Баримтын огноо</span><span>${date.day} / ${date.month} / ${date.year}</span></div></footer><footer class="stock-in-receipt-panel__foot">${excelDownloadBtn("confirmStockInExcel()", { extraClass: "btn--toolbar-block" })}</footer></section>`;
}
...
function stockOutReceiptRow(line) {
  const salesPrice = Number(line.unitPrice) || 0;
  const totalPrice = Number(line.totalPrice) || 0;
  const note = String(line?.note || "").trim();
  const noteHtml = note
    ? `<span class="stock-in-table__note">${esc(note)}</span>`
    : "";
  return `<article class="stock-in-receipt-line"><div class="stock-in-receipt-line__main"><p class="stock-in-receipt-line__name">${esc(line.productName)}</p>${noteHtml}<div class="stock-in-receipt-line__meta"><span class="stock-in-receipt-line__barcode">${esc(line.barcode || "—")}</span>${stockReceiptPackHtml(line)}<span class="stock-in-receipt-line__qty">${line.quantity} ш</span></div></div><div class="stock-in-receipt-line__prices"><span class="stock-in-receipt-line__price"><span class="stock-in-receipt-line__price-label">Үнэ</span><span class="stock-in-receipt-line__price-value">${fmt(salesPrice)}</span></span><span class="stock-in-receipt-line__price stock-in-receipt-line__price--total"><span class="stock-in-receipt-line__price-label">Нийт</span><span class="stock-in-receipt-line__price-value">${fmt(totalPrice)}</span></span></div></article>`;
}
function stockOutReceiptPanel(receipt) {
  ...
  const rows = groups
    ...
    .join("");
  const date = stockInReceiptDateParts(receipt.createdAt);
  return `<section class="stock-in-receipt-panel"><header class="stock-in-receipt-panel__head"><div><p class="stock-in-receipt-panel__title">${esc(stockOutReceiptTitle(receipt))}</p><p class="stock-in-receipt-panel__sub">Ажилтан: ${esc(receipt.employeeName)} · ${(receipt.lines || []).length} бараа</p></div></header><div class="stock-in-receipt-list">${rows}<div class="stock-in-receipt-total"><span class="stock-in-receipt-total__label">Нийт дүн</span><span class="stock-in-receipt-total__value">${fmt(receipt.totalAmount || 0)}</span></div></div><footer class="stock-in-receipt-panel__signatures"><div class="stock-in-sign"><span>Хүлээлгэн өгсөн</span><span class="stock-in-sign__line"></span></div><div class="stock-in-sign"><span>Хүлээн авсан</span><span class="stock-in-sign__line"></span></div><div class="stock-in-sign stock-in-sign--date"><span>Баримтын огноо</span><span>${date.day} / ${date.month} / ${date.year}</span></div></footer><footer class="stock-in-receipt-panel__foot">${excelDownloadBtn("confirmStockOutExcel()", { extraClass: "btn--toolbar-block" })}</footer></section>`;
}
function stockOutModal(id) {
  ...
  box(
    "Зарлага гаргах",
    stockTakeEntryFormHtml(p, d, "out"),
    "max-w-md stock-in-take-modal",
  );
  requestAnimationFrame(() => {
    const form = modal.querySelector("form.stock-in-take");
...
function xlsxCellXml(ref, styleId, value, kind) {
  ...
  // Force plain text (barcodes) — avoids Excel/Numbers scientific notation.
  if (kind === "inlineStr") {
    return `<c r="${ref}" s="${styleId}" t="inlineStr"><is><t xml:space="preserve">${xlsxEscapeText(value)}</t></is></c>`;
  }
...
async function exportProductsExcelXlsx() {
  ...
  await downloadBlobFile(blob, `Бараа-${stamp}.xlsx`);
}
...
async function exportCountExcelXlsx() {
  ...
  zip.file("xl/worksheets/sheet1.xml", sheetXml);
  const blob = await zipToExcelBlob(zip);
  await downloadBlobFile(blob, `Тооллого-${stamp}.xlsx`);
}
...
function buildStockOutSheetXmlLegacy(receipt, {
  ...
  const receivedDateValue = warehouseSheetDateValue(
    receipt.createdAt ? isoDay(receipt.createdAt) || todayIso() : todayIso(),
  );
  const printedDateValue = warehouseSheetDateValue(todayIso());
  const groups = stockInReceiptGroupedLines(receipt.lines);
  const rows = [];
  ...
  const pushSignatureBlock = (roleLabel) => {
    ...
    pushRow(null, [
      ...
      ...emptyCells(signRow, "D", lastCol, 18),
    ]);
  };
...
function warehouseOrderProductsGrouped(orders, opts = {}) {
  ...
  orders.forEach((o) =>
    o.items.forEach((i) => {
      ...
      } else if (productLargeBoxCount(prod) > 1) {
        map[key].largeBoxQuantity = productLargeBoxCount(prod);
      }
    }),
  );
...
function promotionPageTitle(type) {
  return PROMO_PAGE_TITLES[type] || "Урамшуулал";
}
...
function promoProductLabelInline() {
  return labelInline(PROMO_PRODUCT_LABEL);
}
function promoProductQtyLabel(qty) {
  return `${Number(qty) || 1} ш · ${promoProductLabelInline()}`;
}
...
function promotionSearchQtyRow(searchInputHtml, qtyOpts) {
  const search = `<div class="promo-toolbar__search">${searchInputHtml}</div>`;
  if (!qtyOpts) {
    return `<div class="promo-toolbar">${search}</div>`;
  }
  const qtyHtml = promotionQtyField(
    qtyOpts.name,
    qtyOpts.label,
...
function promotionQtyField(name, label, defaultValue, variant = false) {
  ...
  return `${wrap}<label class="${cls}">${labelHtml}<input name="${name}" type="tel" inputmode="numeric" pattern="[0-9]*" autocomplete="off" data-promo-digits="1" required${val} placeholder="${esc(ph)}"${aria} oninput="promoFormDraftField(this)" class="promo-qty-input"></label>${wrapEnd}`;
}
...
function quantityPromoFreeQty(rule, sets = 1) {
  ...
  const n = Math.max(1, Math.floor(Number(sets) || 0));
  return freeQty < 1 ? 0 : freeQty * n;
}
function quantityPromoExampleLine(rule, sets = 1) {
  const order = quantityPromoOrderQty(rule, sets);
  const promo = quantityPromoFreeQty(rule, sets);
...
function warehouseView() {
  state.filters.warehouseTab = "orders";
  return `<div class="space-y-3">${pageHead("Агуулах")}<div class="grid grid-cols-1 gap-3">${workerChooser(warehouseOrdersForSelectedWorkers())}</div></div>`;
}
...
function detailRow(x) {
  const p = x.product;
  const qtyHtml = prepareQtyColsHtml(
    {
...
function clearWorkerOrderEditState() {
  state.editingOrderId = "";
...
function openWorkerOrderEdit(id) {
  ...
  state.editingOrderOriginalItems = (order.items || []).map((i) => ({ ...i }));
  state.workerCustomer = order.customerId || "";
  state.workerStoreReady = true;
...
function cancelWorkerOrderEdit() {
  clearWorkerOrderEditState();
  state.workerStoreReady = false;
  state.workerCustomer = "";
  state.deliveryDate = "";
...
function workerSelectedRow(p) {
  ...
  const remain = Math.max(0, productStockWithEditCredit(p.id) - p.qty);
  const lineTotal = p.price * p.qty;
  const tags = [
    p.category ? `<span>${esc(p.category)}</span>` : "",
    `<span>Үлд ${remain}</span>`,
  ]
    .filter(Boolean)
...
function workerOrders(orders) {
  ...
  return `<section class="worker-orders-panel">${metricsBar(`${card("Захиалга", orderCount)}${card("Нийт дүн", fmt(total))}`, 2)}<div class="line-panel__toolbar worker-orders-filters"><button type="button" onclick="clearWorkerOrderDate()" class="worker-orders-filters__chip${!day ? " is-active" : ""}">Бүгд</button><button type="button" onclick="selectWorkerOrderToday()" class="${todayBtnClass}">Өнөөдөр</button><input type="date" value="${esc(day)}" onchange="setWorkerOrderDate(this.value)" oninput="setWorkerOrderDate(this.value)" onfocus="toolbarSelectFocus()" onblur="toolbarSelectBlur()" class="flex-1 min-w-[140px] px-3 py-2 bg-secondary rounded text-sm app-input" aria-label="Огноо"></div><div class="line-list line-list--scroll">${orders.length ? orders.map(workerOrderListRow).join("") : `<p class="line-panel__empty">${day === today ? "Өнөөдөр захиалга байхгүй" : day ? "Сонгосон өдөр захиалга байхгүй" : "Захиалга байхгүй"}</p>`}</div></section>`;
}
...
function productModal(id) {
  ...
  const largeBoxVal =
    Number(p.largeBoxQuantity) > 1
      ...
      : "";
  const smallBoxAttrs = inputAttrs(smallBoxVal, "10", {
    treatZeroAsEmpty: true,
  });
  const largeBoxAttrs = inputAttrs(largeBoxVal, "10", {
    treatZeroAsEmpty: true,
  });
...
function buildProductDataFromForm(form) {
  ...
  if (!largeRaw) {
    ...
    if (!data.boxQuantity || data.boxQuantity <= 1) {
      return {
...
function orderStockIssues(items, opts = {}) {
  ...
  const credit = opts.stockCredit || {};
  const issues = [];
  Object.entries(need).forEach(([id, qty]) => {
    const p = state.products.find((x) => x.id === id);
    const have = (Number(p?.stock) || 0) + (Number(credit[id]) || 0);
    if (!p || qty > have) {
      issues.push({
        id,
...
function editingOrderStockCreditMap() {
  const map = {};
  if (!state.editingOrderId) return map;
  (state.editingOrderOriginalItems || []).forEach((item) => {
    const id = String(item?.productId || "");
    if (!id) return;
...
function productStockWithEditCredit(productId) {
...
function getWorkerQty(productId) {
  ...
  const p = state.products.find((x) => x.id === productId);
  if (!p) return 0;
  return Math.min(raw, productStockWithEditCredit(productId));
}
...
async function saveWorkerOrderEdit() {
  if (orderSubmitLock) return;
  if (!state.editingOrderId) return saveWorker();
  if (!state.isLoggedIn) return alert("Захиалга хадгалахын өмнө нэвтэрнэ үү");
  const order = state.orders.find((x) => x.id === state.editingOrderId);
  if (!order) {
    clearWorkerOrderEditState();
  ...
  try {
    ...
    order.percentDiscount = order.applyPercentDiscount
      ? percentDiscountRate()
      : 0;
    order.settlementAgreed = !!settlementText;
    order.settlementText = settlementText;
    ...
    state.filters.worker = "orders";
    state.workerOrdersArrived = true;
    state.workerHighlightOrderId = orderId;
    persistOrderSnapshot();
    saveAuthSession();
    render();
    pushAppHistory();
    ...
    if (saved) {
      ...
      showAppToast(
        "Өөрчлөлт хадгалагдлаа, серверт илгээхэд алдаа гарлаа",
        "error",
      );
    }
    requestAnimationFrame(() => {
    ...
    console.warn("Order edit save failed", error);
