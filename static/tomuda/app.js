...
// ҮНДСЭН A–K: A logo; B fits «Дансны дугаар:»; C = «IBAN:»
const RECEIPT_XLSX_COL_WIDTHS = [
  4.5, 15.75, 9.0, 2.875, 12.0, 9.25, 5.5, 4.875, 3.75, 9.25, 9.5,
];
...
function appendReceiptSheetRows(o, ctx, rows, merges, startRow = 1) {
  ...
  merges.push(
    ...
    `B${bankR3}:C${bankR3}`,
    `D${bankR3}:E${bankR3}`,
    // B = Дансны дугаар:; C = IBAN:; D:E = short number
    `D${bankR4}:E${bankR4}`,
    `D${bankR5}:E${bankR5}`,
  );
  pushRow(14.25, [
  ...
  pushRow(perBankH, [
    // 15 = left, no wrap — keeps 9pt size (style 5 wrap was shrinking in tall row)
    xlsxCellXml(`B${bankR4}`, 15, si("Дансны дугаар:"), "s"),
    // 30 = right-aligned IBAN flush to right edge of C
